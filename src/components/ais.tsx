...
function buildBoundsPairs(bounds: AisBounds): [[number, number], [number, number]] {
  return [
...
function isPointBounds(bounds: AisBounds): boolean {
...
export function AisModule({ hardware, onRefreshHardware }: AisModuleProps) {
  ...
  const [snapshot, setSnapshot] = useState<AisFeedSnapshot | null>(null);
  const [selectedMmsi, setSelectedMmsi] = useState("");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [runtimeBusy, setRuntimeBusy] = useState(false);
  const [savedCityView, setSavedCityView] = useState<AisPoint | null>(null);
  ...
  refreshSnapshotRef.current = async () => {
    try {
      const nextSnapshot = await fetchAisFeed();
      setSnapshot(nextSnapshot);
      setError("");
    } catch (pollError) {
      setError(
        pollError instanceof Error ? pollError.message : "Could not refresh the AIS feed.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function controlRuntime(method: "POST" | "DELETE"): Promise<void> {
    ...
    try {
      await requestAisRuntime(method);
      setError("");
    } catch (runtimeError) {
      setError(
        runtimeError instanceof Error
          ...
          : method === "POST"
            ? "Could not start the AIS decoder."
            : "Could not stop the AIS decoder.",
      );
    } finally {
